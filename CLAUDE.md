...
| `ANTHROPIC_AGENT_ID_CL` | Yes | Managed agent ID for Chile (Ley 21.719) |
| `ANTHROPIC_AGENT_ID_EU` | Yes | Managed agent ID for EU (GDPR) |
| `ANTHROPIC_AGENT_ID_US_CA` | Yes | Managed agent ID for California (CCPA) |
| `ANTHROPIC_ENVIRONMENT_ID` | Yes | Environment ID shared across all agents |
| `TAVILY_API_KEY` | Yes | Used by the `search_policy_url` custom-tool handler |
| `JINA_API_KEY` | No | Optional; raises Jina Reader rate limits for `read_url` |

One Anthropic Agent per jurisdiction (Path A): the law text is baked into each Agent's system prompt so it is cached for free across all sessions. See `agent-config/README.md` and the per-jurisdiction files in `agent-config/` for the system prompts and tool JSON to paste into the Anthropic console.
...
laws/                     Static legal texts (markdown). One Agent per jurisdiction is configured at
                          Anthropic with the corresponding file's contents in its system prompt.
agent-config/             Per-jurisdiction system-prompt + tool JSON to paste into the Anthropic
                          console. Generated by `pnpm dlx tsx scripts/build-agent-config.ts`.
scripts/
  build-agent-config.ts   Regenerates agent-config/*.md from laws/*.md.
